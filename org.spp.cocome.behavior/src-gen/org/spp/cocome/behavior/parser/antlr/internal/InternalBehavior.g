...
(
    ...
    { 
        $current = $this_TypedValueReference_1.current; 
        afterParserOrEnumRuleCall();
    }
)
;
...
entryRuleTypedValueReference returns [EObject current=null] 
	:
	{ newCompositeNode(grammarAccess.getTypedValueReferenceRule()); }
	 iv_ruleTypedValueReference=ruleTypedValueReference 
	 { $current=$iv_ruleTypedValueReference.current; } 
	 EOF 
;

// Rule TypedValueReference
ruleTypedValueReference returns [EObject current=null] 
    @init { enterRule(); 
    }
    @after { leaveRule(); }:
(
